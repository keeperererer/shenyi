<template>
  <div class="detail">
    ...
    <div class="detail_right">
      ...
      <p>标准号:<span>{{product.sBiaoZhun}}</span></p>
      <p>类别:<span>{{product.type}}</span></p>
      <p>规格:<span>{{product.sGuiGe}}</span></p>
      <p>机械性能:<span>{{product.sJiXieXingNeng}}</span></p>
      <p>表面处理:<span>{{product.sBiaoMianChuLi}}</span></p>
      <p>材质:<span>{{product.sCaiZhi}}</span></p>
    </div>
...
export default {
  ...
  methods: {
    ...
    cancel() {
      ...
      getProduct() {
        this.$http.get('/apis/wx/products', {pId: this.$route.params.pId}).then(response => {
            let that = this;
      ...
      loadTypeByTid(tId) {
        this.$http.get('/apis/web/getMaintainableTables', {tableName: 'type'}).then(response => {
            let that = this;
            this.dealResponse(response, function () {
                let types = response.data.data;
                for (let i=0; i<types.length; ++i) {
                    if (types[i]['tId'] === tId) that.product['type'] = types[i]['tName'];
                }
            })
        })
      },
